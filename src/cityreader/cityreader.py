...
def cityreader(cities=[]):
    ...
    with open('cities.csv') as csvfile:
        ...
        for row in read_csv:
            ...
            lon = row[4]
            if name != 'city':
                create_city = City(name, lat, lon)
                cities.append(create_city)
...
def check_input(input):
    if len(input.split(',')) == 2:
        ...
        try:
            ...
            print("the values you entered is not right please make sure they are a number")

    else:
        print("the values you entered is not right please separate the lat an long with a ','")


def check_point(value, max_value, min_value):
    return float(max_value) > float(value) > float(min_value)
